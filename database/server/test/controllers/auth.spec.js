...
describe('Authentification controller', () => {
  ...
  /*  it('should sign up a new employee successfully', (done) => {
      const user = { first_name: 'Sarah', email: 'test@teamwork.com', password: 'Test@2018' };
      chai.request(app)
    ...
    it('should throw an error if required field is missing', (done) => {
      const user = { first_name: 'Patience', email: 'sarah.lif1@gmail.com' };
      chai.request(app)
        .post(`${baseUrl2}/auth/signup`)
        .send(user)
        .then((res) => {
   ...
   /* it('should not sign up an employee with a non existing email', (done) => {
  ...
  describe('Login an employee', () => {
    ...
    //   const user = { email: 'test@teamwork.com', password: 'S@Test@2018' };
    //   chai.request(app)
